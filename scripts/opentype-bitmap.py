...
try:
    ...
    raise ImportError("You must have fontforge > 20190413 installed.")
...
def generate(fontname, files, output_dir):
    filename = output_dir + fontname + '.otb'
...
# Add trailling slash to the output dir if needed
output_dir = os.path.join(args.output_dir, '')

if not os.path.isdir(output_dir):
    print("Specified output dir is not valid")
    exit(1)
...
for fontname in fontfiles:
    generate(fontname, fontfiles[fontname], output_dir)
